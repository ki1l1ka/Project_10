...
def load_base_excel_files(folder_path):
    ...
    try:
        ...
        # Считываем таблицы по переданному пути
        return {
...
with col_left:
    ...
    # Показываем текущий выбранный путь к файлам для визуального контроля
    st.divider()
...
with col_center:
    ...
    with tab_table:
        if active_table_obj is not None:
            #Извлекаем матрицу из объектов Cell
            raw_matrix = [[cell.value for cell in row] for row in active_table_obj.matrix]

            if len(raw_matrix) > 0:
                # Определяем реальное максимальное количество колонок
